...
def generate_report(base_path, output_dir,subject_name,template_file_path='longTR_template.html'):
    ...
    # volume, volume_remained, ccthreshold = None, None, None

    for file_info in matching_files:
        if 'threshold' in file_info and 'extent' in file_info:
            threshold = file_info['threshold']
            extent = file_info['extent']
        elif 'volume' in file_info and 'volume_remained' in file_info and 'ccthreshold' in file_info:
    ...
    for threshold in unique_thresholds:
        for extent in unique_extents:
            FLAimages.append({'path': f"data:image/png;base64,{image_to_base64(os.path.join(path, f'{subject_name}_thresh_{threshold}_extent_{extent}.png'))}", 'caption': f"1st level analysis(none,threshold={threshold},cluster wise={extent})"})
    ...
    data = {
        ...
        # 'CCimages': [
        #     {'path': f"data:image/bmp;base64,{image_to_base64(os.path.join(path, f'{subject_name}_cross_correlation.bmp'))}", 'caption': 'cross correlation'},
        ...
        'CCimages': CCimages,
        # 'FLAimages': [
        #     {'path': f"data:image/png;base64,{image_to_base64(os.path.join(path, f'{subject_name}_thresh_{threshold}_extent_{extent}.png'))}",
