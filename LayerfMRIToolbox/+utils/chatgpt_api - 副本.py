...
from langchain_community.embeddings import OpenAIEmbeddings
from langchain_community.vectorstores import FAISS
from langchain_community.document_loaders import DirectoryLoader, TextLoader, PyPDFLoader, UnstructuredPDFLoader, UnstructuredWordDocumentLoader
...
class DocFetcher:
    ...
    def _get_matlab_docs(self, error_info: ErrorInfo) -> str:
        ...
        try:
            ...
            # 提取相关文档内容
            relevant_content = []
            for result in soup.select('.search_result')[:3]:  # 获取前3个搜索结果
...
""")
    ...
    def _get_python_docs(self, error_info: ErrorInfo) -> str:
        ...
        try:
            ...
            relevant_content = []
            for result in soup.select('.search-result-item')[:3]:  # 获取前3个搜索结果
...
class ErrorAnalyzer:
    ...
    def analyze_error(self, error: Exception) -> ErrorInfo:
        ...
        return error_info
        # error_info.suggestion = self._analyze_error_type(error_info)
        # return error_info
    ...
    def _get_python_doc_reference(self, error_info: ErrorInfo) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                soup = BeautifulSoup(response.text, 'html.parser')
                relevant_content = []
                for result in soup.select('.search-result-item')[:2]:  # 获取前2个结果
                    link = result.select_one('a')
...
"""
    ...
    def _get_ai_suggestion(self, error_info: ErrorInfo) -> str:
        ...
        try:
            ...
            # 输出整个返回的响应
            print(response.encode('utf-8').decode('utf-8'))  # 打印整个响应以供调试
            
            # 获取并打印具体内容
            message_content = response.choices[0].message.content
            print(message_content)  # 确保这是你期望的输出格式
            return message_content
            # # 在这里打印响应
            # print("Response from ChatGPT: ")
            # print(response)  # 打印整个返回内容
            # return response.choices[0].message.content
...
class FMRIAssistant:
    ...
    def load_knowledge(self, knowledge_dir: str):
        """加载知识库"""
        self.knowledge_base.load_knowledge(knowledge_dir)
    ...
    def handle_error(self, error: Exception) -> str:
        """处理代码执行错误"""
        try:
            error_info = self.error_analyzer.analyze_error(error)
