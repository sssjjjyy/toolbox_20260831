function LongTR_immediate_analysis(working_dir, filename, TR, prestim_num, stim_num, interstim_num, poststim_num, block_num, duration_input, fwhm_1, thresh_values, cluster_ext_values,FD_thrs,type)

addpath(genpath('D:\code\layerfmritoolbox\layerfmri_code\'))

% filename='C:\Users\DELL\Desktop\no-a\visual\20250703_480\480_7\480_7.nii';
...
% block_num=45; % Number of Task Blocks
% duration_input=10;
% FD_thrs = 1;
% TR=1;%mouse 
% type = 'mouse';
...
if strcmp(type, 'mouse')
    ...
    hrf = spm_hrf(TR, [3.0 4 0.5 0.5 1.5 0 32]); 
elseif strcmp(type, 'rat')
    dis_radius = 5;
    hrf = spm_hrf(TR, [3.5 4 0.5 0.5 1.25 0 32]); 
else
...
gzFilePath  = ['r',scan_name,'.nii.gz'];
gunzip(gzFilePath, results_dir);
...
header.dt(1)=64;
header.fname=['sr',scan_name,'_cc_map_optimized.nii'];
...
header.fname = ['sr',scan_name,'_lag_map.nii'];
...
colorbar('Position', [0.35 0.48 0.3 0.02], 'Orientation', 'horizontal'); % Adjust the position as needed
% colorbar;
export_fig([scan_name,'_cc_unthresholded.png'],'-r','300')
...
colormap(mymap)
set(gcf,'color','black');
export_fig([scan_name,'_thresh_',num2str(thrs),'_cross_correlation.png'],'-r','300');
...
colormap(jet)
set(gcf,'color','black');
export_fig([scan_name,'_lag_map.png'],'-r','300');
...
onset_hrf_scrubbed = conv(onset_scrubbed, hrf);
onset_hrf_scrubbed = onset_hrf_scrubbed(1:length(onset_scrubbed)); % Trim back to original scan length
...
for lag = lags_to_test
    % Shift the HRF-convolved task vector forward by 'lag' TRs
    if lag == 0
        shifted_task = onset_hrf_scrubbed;
    else
        shifted_task = [zeros(1, lag), onset_hrf_scrubbed(1:end-lag)];
    end
    
    % Calculate true partial correlation, controlling for 6 motion parameters
    % Inputs must be columns: (Brain Data, Task Vector, Motion Matrix)
    current_corr = partialcorr(img_2D_brain', shifted_task', motion(retained_points_index));
...
header=EPIfilename_header(1);
header.dt(1)=64;
header.fname=['sr',scan_name,'_cc_map_optimized_scrubbed.nii'];
spm_write_vol(header,corr_3D);
...
lag_1D(img_idx) = best_lag_map;
lag_3D = reshape(lag_1D,[dim(1),dim(2),dim(3)]);
header.fname = ['sr',scan_name,'_lag_map_scrubbed.nii'];
spm_write_vol(header,lag_3D);
...
colormap(jet)
set(gcf,'color','w');
export_fig([scan_name,'_scrubbed_unthresholded_cc.png'],'-r','300');
...
colormap(mymap)
set(gcf,'color','black');
export_fig([scan_name,'_',num2str(retained_block_num),'remained_',num2str(block_num),'_scrubbed_thresh_', num2str(thrs),'_cross_correlation.png'],'-r','300');
...
colormap(jet)
set(gcf,'color','black');
export_fig([scan_name,'_scrubbed_lag_map.png'],'-r','300');
