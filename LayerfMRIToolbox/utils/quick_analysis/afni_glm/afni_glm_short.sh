...
3dDeconvolve -input "${inputfile}" \
             ...
             -stim_file 2 motion_params.1D[0] -stim_base 2 -stim_label 2 roll \
             -stim_file 3 motion_params.1D[1] -stim_base 3 -stim_label 3 pitch \
             -stim_file 4 motion_params.1D[5] -stim_base 4 -stim_label 4 dP \
             -TR_times ${TR} \
             -x1D X.xmat.1D -x1D_stop \
             -xjpeg design_matrix.jpg
...
3dREMLfit -matrix X.xmat.1D \
          ...
          -fout -tout
# # # Step 3: 输出 p 值图
# # 3dcalc -a stats_output_REML+orig'[3]' -expr 'fitt_t2p(a,617)' -prefix p_values_output_REML


# # # 3dttest++一定要多个文件
# # # 3dcalc -a mask.nii -expr 'a' -prefix mask -datum byte

# # 3dttest++ -Clustsim -prefix Auditory_REML_ttest \
# #           -labelA Auditory \
# #           -mask mask+orig \
# #           -setA stats_output_REML+orig[2]

# # #  3dClustSim -acf 的模拟需要至少 4×4×4 个体素的体块 才能进行合理的空间自相关模拟
# # 3dFWHMx -acf -input errts_REML+orig -mask mask+orig > acf.txt
# # 数据在 z 轴（slice 数）太少，只有 3 层，而 3dClustSim -acf 的模拟需要至少 4×4×4 个体素的体块 才能进行合理的空间自相关模拟。
# # 3dClustSim -mask mask+orig -acf  0.993984  0.133268  0.0825381 -both -prefix ClustSim
...
# Step 4: 转换为 NIFTI 格式
# 3dAFNItoNIFTI -prefix p_output.nii p_values_output_REML+orig
