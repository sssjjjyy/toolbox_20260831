function target_file_nii_set = process_MB_data(working_dir, working_subject, task, type, MB_folders, sub_num, bids_root,events)
    % 处理EPI数据
    target_file_nii_set = {};
    for folder_idx = 1:length(MB_folders)
        % 定义源目录
        source_dir = fullfile(working_dir, working_subject, MB_folders, 'pdata', '1', 'dicom');
        % 定义目标目录
        ...
        for i = 1:length(files)
            ...
            % 生成目标文件名
            target_file_basename = ['sub-' num2str(sub_num) '_task-' task];

            series_num = MB_folders;
            series_num = string(series_num);

            if ~isempty(type)
                target_file_basename = [target_file_basename '_' type];
                if(task ~= "rest")
            ...
            target_file_json_basename =  [target_file_basename '_bold.nii'];
